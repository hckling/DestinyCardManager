...
public class DeckBuilder {
    ...
    public Deck getDeck() {
        ...
        if (battlefieldSelector.getSelectionCount() > 0) {
            result.selectBattlefield(battlefieldSelector.getSelected().get(0));
        }

        for(CardSelectionInfo c : eventSelector.getSelected()) {
            result.addEvent(c);
        }

        for(CardSelectionInfo c : supportSelector.getSelected()) {
            result.addSupport(c);
        }

        for (CardSelectionInfo c : upgradeSelector.getSelected()) {
            result.addUpgrade(c);
        }
    ...
    public void loadFromDeck(Deck deck) {
        for (CharacterSelectionInfo c : deck.getSelectedCharacters()) {
            characterSelector.select(c);
        }

        battlefieldSelector.select(deck.getSelectedBattlefield());

        for (CardSelectionInfo c : deck.getSelectedUpgrades()) {
            upgradeSelector.select(c);
        }

        for (CardSelectionInfo c : deck.getSelectedSupport()) {
            supportSelector.select(c);
        }

        for (CardSelectionInfo c : deck.getSelectedEvents()) {
            eventSelector.select(c);
        }
    }
}
